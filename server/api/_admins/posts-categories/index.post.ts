...
import { createPostsCategorySchema } from "~~/server/validators/posts-category";

export default defineEventHandler(async (event) => {
  const body = await readBody(event);
  const parsed = createPostsCategorySchema.safeParse(body);
  ...
  const [result] = await db.insert(postCategories).values({
    name,
    slug,
  });
