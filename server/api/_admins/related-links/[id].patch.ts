...
import { updateRelatedLinkSchema } from "~~/server/validators/related-link";
import { z } from "zod";
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = updateRelatedLinkSchema.safeParse(body);

  if (!parsed.success) {
    return zodErrorResponse(parsed.error);
  }

  await db.update(relatedLinks).set(parsed.data).where(eq(relatedLinks.id, id));

  return successResponse({ id }, "Related link berhasil diperbarui");
});
